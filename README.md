...
4. 平台初稿后从目标读者角度检查信任、理解、相关性和行动阻力；建议必须记录采纳、部分采纳或不采纳及理由。
5. 公众号和小红书共享事实与边界，但分别设计标题、开头、结构、视觉任务和行动。
6. 食物、器械和空间优先自然编辑摄影；抽象机制使用当代数字编辑插画。同组图片不混用媒介，不继承旧图的全局颗粒。
7. Skill 只提供 advisory。它不拥有内容状态、SQLite、审批、renderer、导出、登录或发布权限。
...
- Git tag `v0.1.0` 对应首个冻结版本；`v0.2.0` 加入读者视角反思与建议采纳门。

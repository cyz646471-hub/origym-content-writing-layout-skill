...
一个面向 Codex 的健身健康内容 Skill。它把每日选题雷达，以及候选后的评估、隔离协作、来源约束写作、生图、双重读者反思、移动端排版、最终交付和人工发布回填放在同一个 advisory 适配层中。
...
- 对研究、专业解读、社区故事和行业材料分级，形成可追溯候选，并在明确选题后继续生成双平台图文评审包。
...
- 在可信产品确认当前哈希版本获批后，生成最终本地发布包、人工平台复核清单并接收发布结果回填；Skill 不执行外部平台操作。
...
8. `develop-lead` 自动推进已选候选到双平台评审包；`release-current` 只在可信产品提供当前批准绑定后生成最终本地文件和人工发布清单。
9. Skill 只提供 advisory。它不拥有内容状态、SQLite、审批、renderer 或外部平台操作权限。
...
```text
使用 $origym-content-writing-layout，以 release-current 模式处理 Signal Desk 已明确批准的当前版本；重验内容、资产、renderer 与 Skill 绑定，生成最终本地发布包和人工复核清单，不登录或操作外部平台。
```
...
- Git tag `v0.1.0` 对应首个冻结版本；`v0.2.0` 加入读者视角反思与建议采纳门；`v0.3.0` 纳入每日健身健康选题雷达；`v0.4.0` 固定已选候选从补证、隔离协作、生图、双重反思、排版到人工发布交接的完整流程。

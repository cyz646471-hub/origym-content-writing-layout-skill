...
| 健康 Obsidian | `/home/ethan/CHI-Papers/04-健康情报` | 查相关 `每日简报/`、`来源笔记/`、`主题/` 与合法归档原文；不扫描或改写整个 vault。 |
| 每日健康情报 | `/home/ethan/projects/daily-arrangement/automation/health-obsidian-prompt.md`；运行记录在同项目 `health-obsidian-runs/` | 提供近期候选、来源笔记和主题关系。只作选题补充，不自动改变用户主题。 |
...
| 图片生成与编辑 | `imagegen` | 生成或微调无字源图；使用 `view_image` 查看本地原图。 |
| 唯一视觉模板 | 项目 `docs/ORIGYM-WOMENS-TRAINING-SIX-CARD-TEMPLATE.zh-CN.md` | 3×2 中文卡片、半写实知识分享、米白/炭黑/砖红、ORIGYM 专业健身房和固定人物基线。 |
| 结构参考实现 | 项目 `output/playwright/thin-back-six-card-fixed-v2-native.html` | 只参考区域、网格和真实中文叠排方式；每个主题建立自己的原生 renderer，不复用旧图片或旧文案。 |
| 浏览器出图与复核 | `playwright` Skill | 以实际原生画布截图，并检查资源、越界、中文、遮挡与控制台。 |
| 文件校验 | `sha256sum` | 固定源图、终稿、文案、模板和评审包。 |

## Skill 唯一维护源

- 维护源：`/home/ethan/projects/origym-content-writing-layout-skill/skills/origym-content-writing-layout`
- 当前安装副本：`/home/ethan/.codex/skills/origym-content-writing-layout`
...
`review` 可在 QA 后生成，但始终等待负责人确认。只有当前来源、文案、资产、模板、renderer 与 QA 绑定获明确批准后才写 `final`。
...
- `asset-manifest.json`：文件 SHA、生成方式、模板/renderer 与批准字段；
